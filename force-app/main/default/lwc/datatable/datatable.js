...
import getUsers from "@salesforce/apex/massPermissionsetAssignerCtrl.retrieveUsers";
...
export default class Datatable extends LightningElement {
  ...
  set soslSearchTerm(value) {
    ...
    this.doDataReset();
    if (typeof value === "string" && value.length > 1 && this._initDone)
      this.fetchRecords();
  ...
  constructor() {
    super();
    this.template.addEventListener("selectedpermsets", this.handleNotification);
  }

  handleNotification = (event) => {
    console.log("From Datatable: " + event.detail);
  };
  ...
  handleCancel = (event) =>
    this.dispatchEvent(new CustomEvent("cancel", { detail: event.detail }));
  handleResize = (event) =>
    this.dispatchEvent(new CustomEvent("resize", { detail: event.detail }));
  handleRowSelection = (event) => {
    if (this._originTagRowSelectionLocal === "LIGHTNING-DATATABLE") {
      this.selectedRowsMap = {};
      this.selectedRowsPagesMap[this._paginationInfo.currentPage] =
  ...
  handleSort = (event) => {
    ...
    this._recordsListInAllPages.sort((a, b) => {
      ...
      if (this.tableProps.sortedDirection === "asc") {
        if (a[this.tableProps.sortedBy] < b[this.tableProps.sortedBy])
          return -1;
        else if (a[this.tableProps.sortedBy] > b[this.tableProps.sortedBy])
          return 1;
  ...
  processConfig() {
    if (
      this.config.hasOwnProperty("object-name") ||
      this.config.hasOwnProperty("objectName")
    )
      this.objectName = this.config["object-name"] || this.config.objectName;
    if (
      this.config.hasOwnProperty("sort-by") ||
      this.config.hasOwnProperty("sortBy")
    )
      this.sortBy = this.config["sort-by"] || this.config.sortBy;
    if (
      this.config.hasOwnProperty("sort-asc") ||
      this.config.hasOwnProperty("sortAsc")
    )
      this.sortAsc = this.config["sort-asc"] || this.config.sortAsc;
    if (this.config.hasOwnProperty("limit")) this.limit = this.config.limit;
    if (this.config.hasOwnProperty("cacheable"))
      this.cacheable = this.config.cacheable;
    if (this.config.hasOwnProperty("height")) this.height = this.config.height;

    if (
      this.config.hasOwnProperty("query-type") ||
      this.config.hasOwnProperty("queryType")
    )
      this.queryType = this.config["query-type"] || this.config.queryType;

    if (
      this.config.hasOwnProperty("hide-pagination") ||
      this.config.hasOwnProperty("hidePagination")
    )
      this.hidePagination =
        this.config["hide-pagination"] || this.config.hidePagination;

    if (
      this.config.hasOwnProperty("hide-table-spinner") ||
      this.config.hasOwnProperty("hideTableSpinner")
    )
      this.hideTableSpinner =
        this.config["hide-table-spinner"] || this.config.hideTableSpinner;

    if (
      this.config.hasOwnProperty("user-messages") ||
      this.config.hasOwnProperty("userMessages")
    )
      this.userMessages =
        this.config["user-messages"] || this.config.userMessages;

    if (
      this.config.hasOwnProperty("page-size") ||
      this.config.hasOwnProperty("pageSize")
    )
      this.pageSize = this.config["page-size"] || this.config.pageSize;

    if (
      this.config.hasOwnProperty("query-filters") ||
      this.config.hasOwnProperty("queryFilters")
    )
      this.queryFilters =
        this.config["query-filters"] || this.config.queryFilters;

    if (
      this.config.hasOwnProperty("sosl-search-term") ||
      this.config.hasOwnProperty("soslSearchTerm")
    )
      this.soslSearchTerm =
        this.config["sosl-search-term"] || this.config.soslSearchTerm;

    if (
      this.config.hasOwnProperty("table-config") ||
      this.config.hasOwnProperty("tableConfig")
    ) {
      this.tableConfig = this.config["table-config"] || this.config.tableConfig;
      this.processTableConfig();
      this.fields = this.tableProps.columns
        .filter((col) => col.hasOwnProperty("api"))
        .map((col) => col.api)
        .join();
    }
  }
  ...
  processTableConfig() {
    ...
    if (
      this.tableConfig.hasOwnProperty("hideCheckboxColumn") ||
      this.tableConfig.hasOwnProperty("hide-checkbox-column")
    )
      this.tableProps.hideCheckboxColumn =
        this.tableConfig.hideCheckboxColumn ||
        this.tableConfig["hide-checkbox-column"];
    else this.tableProps.hideCheckboxColumn = false;
    if (
      this.tableConfig.hasOwnProperty("showRowNumberColumn") ||
      this.tableConfig.hasOwnProperty("show-row-number-column")
    )
      this.tableProps.showRowNumberColumn =
        this.tableConfig.showRowNumberColumn ||
        this.tableConfig["show-row-number-column"];
    else this.tableProps.showRowNumberColumn = false;
    if (
      this.tableConfig.hasOwnProperty("rowNumberOffset") ||
      this.tableConfig.hasOwnProperty("row-number-offset")
    )
      this.tableProps.rowNumberOffset =
        this.tableConfig.rowNumberOffset ||
        this.tableConfig["row-number-offset"];
    else this.tableProps.rowNumberOffset = 0;
    if (
      this.tableConfig.hasOwnProperty("resizeColumnDisabled") ||
      this.tableConfig.hasOwnProperty("resize-column-disabled")
    )
      this.tableProps.resizeColumnDisabled =
        this.tableConfig.resizeColumnDisabled ||
        this.tableConfig["resize-column-disabled"];
    else this.tableProps.resizeColumnDisabled = false;
    if (
      this.tableConfig.hasOwnProperty("minColumnWidth") ||
      this.tableConfig.hasOwnProperty("min-column-width")
    )
      this.tableProps.minColumnWidth =
        this.tableConfig.minColumnWidth || this.tableConfig["min-column-width"];
    else this.tableProps.minColumnWidth = "50px";
    if (
      this.tableConfig.hasOwnProperty("maxColumnWidth") ||
      this.tableConfig.hasOwnProperty("max-column-width")
    )
      this.tableProps.maxColumnWidth =
        this.tableConfig.maxColumnWidth || this.tableConfig["max-column-width"];
    else this.tableProps.maxColumnWidth = "1000px";
    if (
      this.tableConfig.hasOwnProperty("resizeStep") ||
      this.tableConfig.hasOwnProperty("resize-step")
    )
      this.tableProps.resizeStep =
        this.tableConfig.resizeStep || this.tableConfig["resize-step"];
    else this.tableProps.resizeStep = "10px";
    if (
      this.tableConfig.hasOwnProperty("defaultSortDirection") ||
      this.tableConfig.hasOwnProperty("default-sort-direction")
    )
      this.tableProps.defaultSortDirection =
        this.tableConfig.defaultSortDirection ||
        this.tableConfig["default-sort-direction"];
    else this.tableProps.defaultSortDirection = "asc";
    if (
      this.tableConfig.hasOwnProperty("enableInfiniteLoading") ||
      this.tableConfig.hasOwnProperty("enable-infinite-loading")
    )
      this.tableProps.enableInfiniteLoading =
        this.tableConfig.enableInfiniteLoading ||
        this.tableConfig["enable-infinite-loading"];
    else this.tableProps.enableInfiniteLoading = false;
    if (
      this.tableConfig.hasOwnProperty("loadMoreOffset") ||
      this.tableConfig.hasOwnProperty("load-more-offset")
    )
      this.tableProps.loadMoreOffset =
        this.tableConfig.loadMoreOffset || this.tableConfig["load-more-offset"];
    else this.tableProps.loadMoreOffset = false;
    if (
      this.tableConfig.hasOwnProperty("isLoading") ||
      this.tableConfig.hasOwnProperty("is-loading")
    )
      this.tableProps.isLoading =
        this.tableConfig.isLoading || this.tableConfig["is-loading"];
    else this.tableProps.isLoading = false;
    if (
      this.tableConfig.hasOwnProperty("maxRowSelection") ||
      this.tableConfig.hasOwnProperty("max-row-selection")
    )
      this.tableProps.maxRowSelection =
        this.tableConfig.maxRowSelection ||
        this.tableConfig["max-row-selection"];
    else this.tableProps.maxRowSelection = 1000;
    if (
      this.tableConfig.hasOwnProperty("selectedRows") ||
      this.tableConfig.hasOwnProperty("selected-rows")
    )
      this.tableProps.selectedRows =
        this.tableConfig.selectedRows || this.tableConfig["selected-rows"];
    ...
    if (
      this.tableConfig.hasOwnProperty("draftValues") ||
      this.tableConfig.hasOwnProperty("draft-values")
    )
      this.tableProps.draftValues =
        this.tableConfig.draftValues || this.tableConfig["draft-values"];
    else this.tableProps.draftValues = null;
    if (
      this.tableConfig.hasOwnProperty("hideTableHeader") ||
      this.tableConfig.hasOwnProperty("hide-table-header")
    )
      this.tableProps.hideTableHeader =
        this.tableConfig.hideTableHeader ||
        this.tableConfig["hide-table-header"];
    else this.tableProps.hideTableHeader = false;
    if (
      this.tableConfig.hasOwnProperty("suppressBottomBar") ||
      this.tableConfig.hasOwnProperty("suppress-bottom-bar")
    )
      this.tableProps.suppressBottomBar =
        this.tableConfig.suppressBottomBar ||
        this.tableConfig["suppress-bottom-bar"];
    ...
    getUsers()
      .then((result) => {
        ...
        this.data = result.map((row) => {
          return {
            ...row,
            permAssignments: this.handlePermissionSets(
              row.PermissionSetAssignments
            )
          };
        });
  ...
  handlePermissionSets(permissionSetAssignments) {
    let permissions = "";
    //console.log(permissionSetAssignments.length);
    if (
      permissionSetAssignments != undefined &&
      permissionSetAssignments.length > 0
    ) {
      for (let i = 0; i < permissionSetAssignments.length; i++) {
        console.log("permissions: " + permissionSetAssignments[i]);
        permissions +=
          i +
          1 +
          "." +
          permissionSetAssignments[i].PermissionSet.Label +
          "<br>";
      }
    }
  ...
  processRecordsResult(recordsListResult) {
    this.handleSpinner(false, "");
    console.log(recordsListResult.length);
    if (recordsListResult && recordsListResult.length > 0) {
      this._recordsListInAllPages = recordsListResult;
      this._paginationInfo.totalPages =
        this._recordsListInAllPages.length / this.pageSize -
        (this._recordsListInAllPages.length % this.pageSize) / this.pageSize +
        (this._recordsListInAllPages.length % this.pageSize === 0 ? 0 : 1);
      this.processRecordsListPagination();
    } else {
      this.doDataReset();
  ...
  processRecordsListPagination(
    lastSetOfRecords = null,
    lastNumberOfRecords = null
  ) {
    if (lastSetOfRecords) {
      this.recordsListInPage = this._recordsListInAllPages.slice(
        lastNumberOfRecords
      );
    } else {
      this.recordsListInPage = this._recordsListInAllPages.slice(
        this._startFromIndex,
        this.pageSize + this._startFromIndex
      );
    }
    ...
    this.recordsListInPage = this.recordsListInPage.map((thisRow) => {
      ...
      this.tableProps.columns.forEach((col) => {
        if (col.hasOwnProperty("api"))
          currentRow[col.fieldName] = this.getFieldValueFromObject(
            currentRow,
            col.api
          );
      });
  ...
  showPreviousPage(event) {
    if (this._startFromIndex > 0) {
      if (
        this.selectedRowsPagesMap.hasOwnProperty(
          this._paginationInfo.currentPage
        ) &&
        this.selectedRowsPagesMap[this._paginationInfo.currentPage].length > 0
      )
        this._originTagRowSelectionLocal = event.target.tagName;
  ...
  showNextPage(event) {
    if (
      this._startFromIndex + this.pageSize <
      this._recordsListInAllPages.length
    ) {
      if (
        this.selectedRowsPagesMap.hasOwnProperty(
          this._paginationInfo.currentPage
  ...
  get pagesInfo() {
    if (this._recordsListInAllPages.length > 0) {
      this._paginationInfo.currentPage =
        (this._startFromIndex + 1) / this.pageSize -
        ((this._startFromIndex + 1) % this.pageSize) / this.pageSize +
        ((this._startFromIndex + 1) % this.pageSize === 0 ? 0 : 1);
      return (
        "Page " +
        this._paginationInfo.currentPage +
        " of " +
  ...
  get recordsInfo() {
    if (this._recordsListInAllPages.length > 0) {
      ...
      return (
        "Showing " +
        (this._startFromIndex + 1) +
        " to " +
        (this._endIndex > this._recordsListInAllPages.length
          ? this._recordsListInAllPages.length
          : this._endIndex) +
        " of " +
        this._recordsListInAllPages.length +
